...
\f\r"'\`<>=]|("|')|))|$)`, "g"),
  ...
  st = (s, e) => {
    ...
    for (let l = 0; l < t; l++) {
      ...
      i += r === z ? c + et : f >= 0 ? (n.push(w), c.slice(0, f) + "$lit$" + c.slice(f) + O + P) : c + O + (f === -2 ? (n.push(void 0), l) : P)
    }
...
class $ {
  ...
  function Y(a) {
    ...
    for (var j = 0, oe = h.length; j < oe; j++) {
      var W = d.get(h[j]),
        se = W.adopters,
        R = W.actions,
        ee = se.get(a),
        Be = p.get(ee);
      if (R.length > 0) {
        for (var ie = Be, Fe = R.length; ie < Fe; ie++) {
          var Se = R[ie],
            Je = Se[0],
            Ke = Se[1];
