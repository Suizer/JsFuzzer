(async function() {
  ...
  function I(n) {
    ...
    if (document.cookie && document.cookie !== "") {
      ...
      for (let f = 0; f < u.length; f += 1) {
        const m = u[f].replace(/^\s+/, "")
          .replace(/\s+$/, "");
        if (m.substring(0, n.length + 1) === `${n}=`) {
          try {
            e = decodeURIComponent(m.substring(n.length + 1)
              .replace(/\+/g, " "))
          } catch (v) {
  ...
  const b = n => {
    ...
    E = (n = 5, e = 500) => new Promise(u => {
      let f = 0;
      const m = I("tracking-info"),
        v = setInterval(() => {
          f++, (m || f > n) && (clearInterval(v), u(m || ""))
        }, e)
    }),
    D = n => {
    ...
    N = _.join("|"),
    B = window.location.href.match(new RegExp(`(${N})`)),
    C = ["/c/newsletter-abmeldung"],
    K = 5e3,
    L = "t/cdn/tracking-components/web-push-sdk/domainSetup",
    w = window.mindShiftNav && window.mindShiftNav.topDomain ? window.mindShiftNav.topDomain : new URL(window.location.href)
    .hostname.split(".")
    .pop(),
    S = `${window.location.origin}/${L}/${w}.json`,
    g = await E();
  try {
    ...
    const e = await n.json(),
      u = () => {
        const {
          pathname: c
        } = window.location, l = c.match(/(\/u\/|\/p\/|\/c\/|\/$)/), s = C.some(d => c.indexOf(d) !== -1);
        return l && !s
      },
      f = (c, l, s, d) => {
        const o = "batchSDK";
    ...
    if (!B) return;
    const m = sessionStorage.getItem("web-push-delay-init"),
      v = Date.now();
    let y = 1e4;
    m ? y = y - (Date.now() - m) : sessionStorage.setItem("web-push-delay-init", v + ""), f(window, document, "script", "https://via.batch.com/v3/bootstrap.min.js"), document.addEventListener("datalayerservice:ga4", c => {
      const {
        detail: l
      } = c, {
        event: s,
      ...
      try {
        ...
        batchSDK(t => {
          t.on("uiReady", async p => {
            ...
            !h && g ? k(g) : h && !g && t.setCustomUserID(null), a.permission === "default" && u() && setTimeout(() => {
              p.ui.show("alert"), l()
            }, K)
